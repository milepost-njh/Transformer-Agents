...
def check_env():
    ...
    if torch.cuda.is_available():
        gpu_count = torch.cuda.device_count()
        logger.info(
            f"✅ 检测到 {gpu_count} 张GPU: {torch.cuda.get_device_name(0)}, bf16支持: {torch.cuda.is_bf16_supported()}")
...
def test_dataloaders(train_loader, val_loader, show_val: bool = True):
    ...
    logger.info(
        f"✅ DataLoader测试通过: batch_size={batch['pt_input_ids'].shape[0]}, seq_len={batch['pt_input_ids'].shape[1]}")
