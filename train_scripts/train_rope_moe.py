...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, dff: int, rate: float = 0.1, use_rope: bool = True, use_moe: bool = False, moe_config=None):
        super().__init__()
...
class EncoderModel(nn.Module):
    def __init__(self, num_layers: int, input_vocab_size: int, max_length: int,
        ...
        self.encoder_layers = nn.ModuleList(
            [EncoderLayer(d_model, num_heads, dff, rate, use_rope=self.use_rope, use_moe=use_moe, moe_config=moe_config) for _ in range(num_layers)]
        )
...
class DecoderModel(nn.Module):
    ...
    def __init__(self, num_layers: int, target_vocab_size: int, max_length: int,
        ...
        self.decoder_layers = nn.ModuleList(
            [DecoderLayer(d_model, num_heads, dff, rate, use_rope=self.use_rope, use_moe=use_moe, moe_config=moe_config) for _ in range(num_layers)]
        )
        ...
        try:
            ...
            for batch_idx, batch in enumerate(train_loader):
                loss_val, acc_val = train_step(
                    batch=batch, transformer=model, optimizer=optimizer, scheduler=scheduler, device=device, moe_config=moe_config
                )
