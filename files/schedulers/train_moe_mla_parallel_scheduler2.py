...
def plot_customized_lr_curve(optimizer, scheduler, total_steps: int, label: str = None):
    ...
    ⚠️ 警告：为了不破坏原始scheduler的状态，这个函数会创建一个新的scheduler副本用于绘图
    ...
    """
    import copy
    # 创建一个新的optimizer和scheduler副本用于绘图，避免破坏原始scheduler
    temp_model = nn.Linear(1, 1)  # 临时模型
    temp_optimizer = type(optimizer)(temp_model.parameters(), **optimizer.defaults)
    
    # 根据scheduler类型创建副本
    if hasattr(scheduler, '_scheduler'):  # ReduceLROnPlateauWrapper
        # 对于ReduceLROnPlateau，使用简化的绘图
        lrs = [optimizer.param_groups[0]['lr']] * total_steps
        logger.warning("ReduceLROnPlateau调度器无法预先绘制学习率曲线，显示恒定学习率")
    else:
        # 为其他调度器创建副本
        temp_scheduler = type(scheduler)(temp_optimizer, **scheduler.__dict__)
        lrs = []
...
def train_step(batch, transformer, optimizer, scheduler=None, device=None, moe_config=None, use_multi_gpu=False):
    ...
    if scheduler is not None:
        # 处理不同类型的调度器
        if hasattr(scheduler, '_scheduler'):  # ReduceLROnPlateauWrapper
            # 对于ReduceLROnPlateau，在训练时不调用step()，在验证时处理
            pass
        else:
            # 对于其他调度器，在训练时调用step()
            scheduler.step()
        ...
        try:
            ...
            if scheduler is not None:
                if hasattr(scheduler, '_scheduler'):  # ReduceLROnPlateauWrapper
                    ...
                    # 对于其他调度器，在训练时已经调用了step()，这里不需要再次调用
                    pass
...
def save_ckpt(model, optimizer, scheduler, epoch, step, ckpt_dir="checkpoints", tag="latest", use_multi_gpu=False):
    ...
    if scheduler is not None:
        try:
            ...
            # 如果调度器没有state_dict方法，记录警告但不中断训练
            logger.warning("调度器不支持state_dict()方法，将不保存调度器状态")
            scheduler_state = None
...
def load_ckpt(model, optimizer=None, scheduler=None, ckpt_dir="checkpoints", device="cpu", use_multi_gpu=False):
    ...
    if scheduler and ckpt["sched"]: 
        try:
            ...
            logger.warning("调度器不支持load_state_dict()方法，将不加载调度器状态")
        except Exception as e:
            logger.warning(f"加载调度器状态时出错: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        default='moe_cosine',
        choices=['transformers_cosine', 'cosine_warmup', 'onecycle', 'reduce_on_plateau', 'moe_cosine'],
        help='学习率调度器类型 (默认: moe_cosine)'
    )
    parser.add_argument(
        '--checkpoint_dir',
        type=str,
    ...
    special_tokens = ["<s>", "<pad>", "</s>", "<unk>", "<mask>"]
    checkpoint_dir = args.checkpoint_dir  # 从命令行参数读取
    ...
    # 调度器配置（从命令行参数读取）
    scheduler_type = args.scheduler  # 可选: "transformers_cosine", "cosine_warmup", "onecycle", "reduce_on_plateau", "moe_cosine"
    ...
    # 6.2 【测试】 打印自定义学习率曲线
    # ⚠️ 警告：plot_customized_lr_curve会破坏scheduler状态，导致训练失效！已注释掉
    ...
    logger.info(f"⚠️ 已跳过学习率曲线绘图以避免破坏scheduler状态")
